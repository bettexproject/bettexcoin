...
#include "zbettexcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBTXC ought to be enough for anybody." - Bill Gates, 2017
    ui->zBTXCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBTXCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBTXC->setEnabled(false);
        ui->pushButtonMintzBTXC->setToolTip(tr("zBTXC is currently disabled due to maintenance."));

        ui->pushButtonSpendzBTXC->setEnabled(false);
        ui->pushButtonSpendzBTXC->setToolTip(tr("zBTXC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBTXCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBTXC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBTXC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBTXC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBTXC in ") +
...
void PrivacyDialog::on_pushButtonSpendzBTXC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBTXC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBTXC
        sendzBTXC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBTXC
    sendzBTXC();
}

void PrivacyDialog::on_pushButtonZBettexControl_clicked()
{
    ZBettexControlDialog* zBettexControl = new ZBettexControlDialog(this);
    zBettexControl->setModel(walletModel);
    zBettexControl->exec();
}

void PrivacyDialog::setZBettexControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBettexSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBTXC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Bettex Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zBTXCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBTXC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BTXC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBTXC</b>";
    ...
    // use mints from zBettex selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBettexControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBettexControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbettex selector in case it was used
    ZBettexControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBettex Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Bettex, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBettex Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBTXC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBTXC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBTXC "));
    ui->labelzBTXCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBTXCSyncStatus->setVisible(fShow);
}
