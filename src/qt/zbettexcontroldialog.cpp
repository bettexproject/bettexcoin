...
#include "ui_zbettexcontroldialog.h"
...
std::list<std::string> ZBettexControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBettexControlDialog::listMints;

ZBettexControlDialog::ZBettexControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBettexControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBettexControlDialog::~ZBettexControlDialog()
{
    delete ui;
}

void ZBettexControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBettexControlDialog::updateList()
{
...
void ZBettexControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBettexControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBettex_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBettexControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBettexControlDialog::GetSelectedMints()
{
...
void ZBettexControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
