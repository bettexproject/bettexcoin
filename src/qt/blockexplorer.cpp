...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::BTXC, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (bettex.conf).");
            QMessageBox::warning(this, "Bettex Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#f8f6f6; bgcolor:#8CAB8A;}\n a, span { font-family: monospace; }\n span.addr {color:#8CAB8A; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: #8CAB8A;}\n td.d0 {font-weight: bold; color:#f8f6f6;}\n h2, h3 { white-space:nowrap; color:#8CAB8A;}\n a { color:#FFFC00; text-decoration:none; }\n a.nav {color:#8CAB8A;}\n";
