...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 1000 BTXC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zBTXC send
enum ZerocoinSpendStatus {
    ZBTXC_SPEND_OKAY = 0,                            // No error
    ZBTXC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZBTXC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZBTXC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZBTXC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZBTXC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZBTXC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZBTXC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZBTXC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZBTXC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZBTXC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZBTXC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZBTXC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZBTXC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZBTXC_SPENT_USED_ZBTXC = 14                       // Coin has already been spend
};
...
public:
    ...
    */
    ...
    void setZBettexAutoBackups(bool fEnabled)
    {
